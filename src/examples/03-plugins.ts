...
console.log("\n--- Custom Parse Format Plugin ---");
// Example 1: Parsing a full date
const fromCustomFormat: TemporalWrapper = atemporal.fromFormat(
  "25/12/2024",
  "DD/MM/YYYY"
);
console.log(`Parsed "25/12/2024" -> ${fromCustomFormat.format("YYYY-MM-DD")}`);

// Example 2: Demonstration of bug fix (parsing time only)
// If no date is provided, atemporal will use the current date.
const fromTimeOnly = atemporal.fromFormat("830", "Hmm");
console.log(
  `Parsed "830" with "Hmm" format -> ${fromTimeOnly.format(
    "YYYY-MM-DD h:mm A"
  )}`
);
...
console.log(
  `Parsed "14:45" with "HH:mm" format -> ${fromTimeOnly2.format(
    "YYYY-MM-DD HH:mm"
  )}`
);
...
console.log(
  `Humanized duration (es-ES): "${atemporal.humanize(myDuration, options)}"`
);
...
console.log(
  `Formatted with ordinal (en-US): "${today.format("Do [of] MMMM, YYYY")}"`
);
console.log(
  `Formatted with ordinal (es-ES): "${today.format(
    "Do [de] MMMM [de] YYYY",
    "es"
  )}"`
);
...
console.log(
  `Default start of week: ${aWednesday
    .startOf("week")
...
console.log(
  `New start of week: ${aWednesday.startOf("week").format("dddd, YYYY-MM-DD")}`
);

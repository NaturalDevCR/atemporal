...
// Cache for regular expressions and format results
class FormatCache {
  ...
  static getRegexForFormat(
    formatString: string
  ): { regex: RegExp; tokens: string[] } | null {
    ...
    try {
      ...
      const tokenRegexPattern = RegexCache.getPrecompiled(
        "customFormatTokenRegex"
      )!;

      // Build the regex pattern for the format
      const regexString = safeFormatString.replace(
        tokenRegexPattern,
        (match, literal) => {
...
export const setCurrentDateFunction = (
...
function parseToISO(dateString: string, formatString: string): string | null {
  ...
  const minute = dateParts.mm
    ? parseInt(dateParts.mm)
    : dateParts.m
    ? parseInt(dateParts.m)
    : 0;
  const second = dateParts.ss
    ? parseInt(dateParts.ss)
    : dateParts.s
    ? parseInt(dateParts.s)
    : 0;
...
const customParseFormatPlugin: Plugin = (
  Atemporal,
...
function getDayFromDayOfYear(
  ...
  try {
    ...
    const date = Temporal.PlainDate.from({ year, month: 1, day: 1 }).add({
      days: dayOfYear - 1,
    });

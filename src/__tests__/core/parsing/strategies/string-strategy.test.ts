...
describe("StringParseStrategy", () => {
  ...
  const createTestContext = (input: any) =>
  ...
  describe("basic properties", () => {
    ...
    it("should have meaningful description", () => {
      expect(strategy.description).toBe(
        "Parse string representations of dates and times"
      );
    });
  });
  ...
  describe("canHandle", () => {
    ...
    it("should not handle non-strings", () => {
      const nonStrings = [123, new Date(), null, undefined, {}, [], true];
    ...
    it("should handle strings with leading/trailing whitespace", () => {
      const inputs = ["  2023-12-25T10:30:00Z  ", "\t2023-12-25\n", " now "];
  ...
  describe("getConfidence", () => {
    ...
    it("should return very low confidence for unrecognized strings", () => {
      const unrecognizedInputs = ["not-a-date", "random-string", "abc123"];
  ...
  describe("validate", () => {
    ...
    it("should warn about undefined/null in strings", () => {
      ...
      inputs.forEach((input) => {
        ...
        expect(result.warnings).toContain(
          "String contains undefined or null values"
        );
      });
    });
    ...
    it("should validate ISO string components strictly", () => {
      const invalidComponents = [
        { input: "2023-13-01T10:00:00Z", error: "Invalid month" },
        { input: "2023-12-32T10:00:00Z", error: "Invalid day" },
        { input: "2023-12-01T24:00:00Z", error: "Invalid hour" }, // 24 is technically valid in some ISO specs as end of day but Temporal might reject or validate logic here check <24
        { input: "2023-12-01T23:60:00Z", error: "Invalid minute" },
        { input: "2023-12-01T23:59:60Z", error: "Invalid second" },
        { input: "2023-02-30T10:00:00Z", error: "does not exist" },
        { input: "2023-04-31T10:00:00Z", error: "does not exist" },
      ];

      invalidComponents.forEach(({ input, error }) => {
        const result = strategy.validate(input, createTestContext(input));
        expect(result.isValid).toBe(false);
        expect(result.errors.some((e) => e.includes(error))).toBe(true);
      });
    });

    it("should validate simple YYYY-MM-DD components", () => {
      const invalidDates = [
        { input: "2023-13-01", error: "Invalid month" },
        { input: "2023-12-32", error: "Invalid day" },
        { input: "2023-02-30", error: "does not exist" },
        { input: "2023-04-31", error: "does not exist" },
      ];

      invalidDates.forEach(({ input, error }) => {
        const result = strategy.validate(input, createTestContext(input));
        expect(result.isValid).toBe(false);
        expect(result.errors.some((e) => e.includes(error))).toBe(true);
      });
  ...
  describe("normalize", () => {
    it("should normalize time-only strings", () => {
      const timeOnly = "09:30:00";
      const testContext = createTestContext(timeOnly); // UTC
      const result = strategy.normalize(timeOnly, testContext);

      const normalized = result.normalizedInput as string;
      expect(normalized).toContain("T09:30:00");
      expect(result.appliedTransforms).toContain("time-only-to-iso");
    });

    it("should normalize time-only strings (HH:mm)", () => {
      const timeOnly = "09:30";
      const testContext = createTestContext(timeOnly);
      const result = strategy.normalize(timeOnly, testContext);

      expect(result.normalizedInput).toContain("T09:30");
    });
    ...
    it("should handle special string values", () => {
      ...
      testCases.forEach(({ input, shouldTransform }) => {
        ...
        if (shouldTransform) {
          ...
          expect(result.normalizedInput).toMatch(
    ...
    it("should track metadata correctly", () => {
      ...
      expect(result.metadata.originalLength).toBe(input.length);
      expect(result.metadata.finalLength).toBe(
        typeof result.normalizedInput === "string"
          ? result.normalizedInput.length
          : 0
      );
      expect(result.metadata.transformCount).toBe(
        result.appliedTransforms.length
      );
    });
  });
  ...
  describe("parse", () => {
    ...
    it("should parse human-readable dates", () => {
      const inputs = ["Dec 25, 2023", "December 25, 2023", "2023/12/25"];
    ...
    it("should handle parsing errors gracefully", () => {
      const invalidInputs = ["invalid-date", "not-a-date", "abc123", ""];
  ...
  describe("checkFastPath", () => {
    ...
    it("should not use fast path for other formats", () => {
      const inputs = ["2023-12-25", "1703505000000", "Dec 25, 2023", "now"];
  ...
  describe("getOptimizationHints", () => {
    ...
    it("should provide medium complexity for human readable", () => {
      const humanInputs = ["Dec 25, 2023", "December 25, 2023"];
    ...
    it("should provide high complexity for unrecognized formats", () => {
      const unrecognizedInputs = ["not-a-date", "random-string"];
      ...
      unrecognizedInputs.forEach((input) => {
        ...
        expect(hints.warnings).toContain(
          "Unrecognized string format may require complex parsing"
        );
      });
    });
    ...
    it("should warn about very long strings", () => {
      ...
      expect(hints.warnings).toContain(
        "Very long string - consider preprocessing"
      );
    });
    ...
    it("should recommend against caching for low confidence", () => {
      ...
      expect(hints.warnings).toContain(
        "Low confidence parsing - results may not be cacheable"
      );
    });
  });
  ...
  describe("edge cases and error handling", () => {
    it("should handle very long strings", () => {
      ...
      expect(() =>
        strategy.getConfidence(longString, testContext)
      ).not.toThrow();
    });
    ...
    it("should parse legacy date strings with offset", () => {
      const input = "Dec 25, 2023 10:30:00 -05:00";
      const testContext = createTestContext(input);
      const result = strategy.parse(input, testContext);
    ...
    it("should handle date-only string without timezone in context", () => {
      const input = "2023-12-25";
      const testContext = createTestContext(input);
      (testContext.options as any).timeZone = undefined;

      const result = strategy.parse(input, testContext);
      expect(result.success).toBe(true);
    });

    it("should handle malformed string matching ISO pattern in fast path", () => {
      const input = "2023-02-30T10:00:00Z";
      const result = strategy.checkFastPath(input, createTestContext(input));
      expect(result.canUseFastPath).toBe(false);
    });
  ...
  describe("integration with Temporal API", () => {
    ...
    it("should handle leap years correctly", () => {
      ...
      const validResult = strategy.parse(
        leapYearInputs[0],
        createTestContext(leapYearInputs[0])
      );
      expect(validResult.success).toBe(true);
      const invalidResult = strategy.parse(
        leapYearInputs[1],
        createTestContext(leapYearInputs[1])
      );
  ...
  describe("Targeted Coverage Improvements", () => {
    it("should hit invalid leap day check (lines 441-445)", () => {
      // 2023 is not a leap year, so Feb 29 is invalid.
      // Date() might accept it and roll over to Mar 1, but we want to trigger the check.
      const result = strategy.parse(
        "2023-02-29",
        createTestContext("2023-02-29")
      );
      expect(result.success).toBe(false);
      expect(result.error?.message).toContain("Invalid leap day");
    });

    it("should hit Date fallback with offset parsing (lines 458-463)", () => {
      const originalZDTFrom = Temporal.ZonedDateTime.from;
      const originalInstantFrom = Temporal.Instant.from;

      (Temporal as any).ZonedDateTime.from = () => {
        throw new Error("Mock fail");
      };
      (Temporal as any).Instant.from = () => {
        throw new Error("Mock fail");
      };

      try {
        // Use a simpler offset string that works with Date
        const input = "2023-01-01T10:00:00+05:00";

        const result = strategy.parse(input, createTestContext(input));
        expect(result.success).toBe(true);
        expect(result.data?.offset).toBe("+05:00");
      } finally {
        (Temporal as any).ZonedDateTime.from = originalZDTFrom;
        (Temporal as any).Instant.from = originalInstantFrom;
      }
    });

    it("should hit checkFastPath failures (lines 509-514)", () => {
      // canHandle returns false -> checkFastPath returns false
      const input = 12345;
      const result = strategy.checkFastPath(input, createTestContext(input));
      expect(result.canUseFastPath).toBe(false);
      expect(result.confidence).toBe(0);
    ...
    it("should hit checkFastPath catch blocks (lines 547-548, 580-581)", () => {
      // Mock Temporal to throw during fast path checks
      const originalZDTFrom = Temporal.ZonedDateTime.from;
      const originalInstantFrom = Temporal.Instant.from;

      // First catch block (lines 547-548) is for ISO_DATETIME_TZ
      (Temporal as any).ZonedDateTime.from = () => {
        throw new Error("Mock fail ZDT");
      };
      (Temporal as any).Instant.from = () => {
        throw new Error("Mock fail Instant");
      };

      const inputTZ = "2023-01-01T10:00:00+05:00";

      let result = strategy.checkFastPath(inputTZ, createTestContext(inputTZ));
      expect(result.canUseFastPath).toBe(false);

      // Second catch block (lines 580-581) is for ISO_DATETIME
      const inputNoTZ = "2023-01-01T10:00:00";
      const ctx = createTestContext(inputNoTZ);
      (ctx.options as any).timeZone = "UTC";
      result = strategy.checkFastPath(inputNoTZ, ctx);
      expect(result.canUseFastPath).toBe(false);

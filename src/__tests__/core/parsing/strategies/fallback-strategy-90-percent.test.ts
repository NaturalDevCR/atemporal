...
describe("FallbackStrategy - Targeted Line Coverage", () => {
  ...
  describe("Validation Edge Cases", () => {
    it("should handle null/undefined in canHandle/getConfidence", () => {
      expect(strategy.canHandle(null as any, context)).toBe(false);
      expect(strategy.getConfidence(null as any, context)).toBe(0);
    });
    ...
    it("should validate function input", () => {
    ...
    it("should validate plain object without useful methods", () => {
      // Object.create(null) has no properties, not even toString
      const result = strategy.validate(Object.create(null) as any, context);
      expect(result.isValid).toBe(false);
    });

    it("should validate complex object without useful methods", () => {
      // Create object with no prototype (no toString)
      const obj = Object.create(null);
      // Give it a constructor so it fails isPlainObject check
      obj.constructor = class Custom {};

      const result = strategy.validate(obj as any, context);
      expect(result.isValid).toBe(false);
      expect(result.errors[0]).toContain(
        "Object input must have toDate(), valueOf(), or toString() method"
      );
    });

    it("should validate plain object with throwing accessors", () => {
      const obj = {};
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "Plain object input without useful conversion methods is not supported for temporal values"
      );
    });

    it("should validate non-plain object with throwing accessors", () => {
      class Custom {}
    ...
    it("should validate non-plain object without methods", () => {
      class Custom {}
      const obj = new Custom();
      // Shadow simple methods with non-functions
      (obj as any).toDate = undefined;
      (obj as any).valueOf = null;
      (obj as any).toString = 123;
  ...
  describe("Object Normalization Edge Cases", () => {
    ...
    it("should use JSON.stringify when toString throws", () => {
      ...
      const result = strategy.normalize(
        objWithThrowingToString as any,
        context
      );
    ...
    it("should use JSON.stringify when String conversion fails", () => {
      ...
      const result = strategy.normalize(
        objWithThrowingStringConversion as any,
        context
      );
      expect(result.appliedTransforms).toContain("object-json-stringify");
    });
  ...
  describe("Parse Method Success Paths", () => {
    it("should parse legacy date string (Dec 25 2023)", () => {
      const result = strategy.parse("Dec 25 2023", context);
    ...
    it("should parse object with toDate()", () => {
      const input = {
        toDate: () => new Date("2023-01-01T00:00:00Z"),
      };
      const result = strategy.parse(input as any, context);
      expect(result.success).toBe(true);
      expect(result.data?.year).toBe(2023);
    });

    it("should parse object with valueOf() returning number", () => {
      const input = {
    ...
    it("should parse BigInt input", () => {
      const input = BigInt(1672531200000);
      const result = strategy.parse(input as any, context);
      expect(result.success).toBe(true);
      expect(result.data?.year).toBe(2023);
    });
    ...
    it("should handle unsupported normalized input type", () => {
      ...
      try {
        ...
        expect(result.error?.message).toContain(
          "Cannot parse normalized input of type"
        );
      } finally {
  ...
  describe("Number Parsing Edge Cases", () => {
    ...
    it("should handle failure to parse as millisecond timestamp", () => {
      ...
      const originalFromEpochMilliseconds =
        Temporal.Instant.fromEpochMilliseconds;
      Temporal.Instant.fromEpochMilliseconds = jest
        .fn()
        .mockImplementation(() => {
  ...
  describe("Additional Coverage Improvements", () => {
    it("should throw when complex object fails both String conversion and JSON.stringify", () => {
      const cyclicObj: any = {};
      cyclicObj.self = cyclicObj; // Causes JSON.stringify to throw
      cyclicObj.toString = () => {
        throw new Error("toString failed");
      };
      cyclicObj.valueOf = () => {
      ...
      expect(() => {
        strategy.normalize(cyclicObj, context);
      }).toThrow("Complex object cannot be converted to temporal value");
    });

    it("should handle parseAsString failure for all methods (lines 459-500)", () => {
      // Mock Temporal methods to ensure they throw even for valid-looking strings if needed,
      // or use a string that guarantees failure like 'invalid-string' which Date/Number also reject.
      const input = "invalid-temporal-string";
    ...
    it("should handle Date parsing fallback validation (lines 481-483)", () => {
      // We need a string that Date can parse but is invalid for Temporal,
      // AND we need to hit the line 481 check !isNaN(date.getTime())
      // Actually we want a success here to hit line 483 (return statement)

      // Mock Temporal failures
      ...
      try {
        // A string that works for Date but would fail mocked Temporal
    ...
    it("should handle generic object returning [object Object] then failing JSON.stringify", () => {
      // Line 286-291
      const cyclicObj: any = {};
      cyclicObj.self = cyclicObj;
      // Default toString returns [object Object]

      // normalize should try JSON.stringify, fail, then fallback to [object Object] string
      const result = strategy.normalize(cyclicObj, context);
      expect(result.normalizedInput).toBe("[object Object]");

...
function buildMethodIndex() {
  ...
  const entries = [
    ...
    { kind: 'static', name: 'atemporal(input?, timeZone?)', sig: '() => TemporalWrapper', note: 'Create an instance. input may be ISO string, Date, Unix ms, Unix s, [y,m,d,h,m,s,ms], plain object, Firebase Timestamp, or another TemporalWrapper.' },
    { kind: 'static', name: 'atemporal.try(input?, timeZone?)', sig: '() => TemporalWrapper | null', note: 'Same as atemporal(input) but returns null instead of throwing on invalid input.' },
    { kind: 'static', name: 'atemporal.iso(input?, timeZone?)', sig: '() => string | null', note: 'Try to parse and return an ISO 8601 string, or null. Does not throw.' },
    { kind: 'static', name: 'atemporal.validate(input)', sig: '() => ValidationResult', note: 'Detailed validation: { valid: boolean, error?: string, code?: ATEMPORAL_* }.' },
    ...
    { kind: 'static', name: 'atemporal.lazyLoad(nameOrMap, timeZone?)', sig: 'async (...) => Promise<TemporalWrapper | Record<string, TemporalWrapper>>', note: 'Lazy-load one or more plugins and the temporal polyfill on demand. Resolves with the instance(s).' },
    { kind: 'static', name: 'atemporal.lazyLoadMultiple(modules)', sig: 'async (...) => Promise<void>', note: 'Preload a list of plugin modules without creating instances.' },
    { kind: 'static', name: 'atemporal.isPluginLoaded(name)', sig: '(name: string) => boolean', note: 'Has the named plugin been installed yet?' },
    { kind: 'static', name: 'atemporal.getLoadedPlugins()', sig: '() => string[]', note: 'Names of currently installed plugins.' },
    { kind: 'static', name: 'atemporal.getAvailablePlugins()', sig: '() => string[]', note: 'Names of plugins registered with the loader (whether installed or not).' },
...
function buildTldr() {
  return [
    ...
    '',
    '- **Install:** `npm install atemporal` (pulls `@js-temporal/polyfill` automatically).',
    '- **Import:** `import atemporal from "atemporal";` (default export, callable).',
    '- **Construct:** `atemporal()` (now) / `atemporal(input, tz?)` / `atemporal.unix(s)` / `atemporal.validate(x)`.',
    '- **Immutable:** every mutator returns a NEW `TemporalWrapper`. Never use `let` to "update" a date — use `const next = current.add(1, "day");`.',
    '- **Plugins:** `atemporal.extend(relTime); atemporal.extend(customParseFormat); ...` (sync) or `await atemporal.lazyLoad({...})` (async).',
    '- **Time zones:** always pass an explicit IANA TZ to functions that compare or persist. Relying on the host TZ is a bug.',
    '- **Errors:** throw `atemporal.errors.InvalidDateError` (carries a stable `code` like `ATEMPORAL_INVALID_FORMAT`). Catch on the boundary, not deep in business logic.',
    ...
    '- **Bundle:** ~15 KB minified ESM, ~8 KB with tree-shaking. No transitive runtime deps.',
